...
const App = () => {
	...
	const getTeamData = async () => {
		...
		result.data.forEach(team => {
			teamsArray.push({ text: team.name, value: team.name, key: team.id })
		});
		setTeams(teamsArray);
		setLoading(false)
	...
	const changeTeam = (e) => {
		setSelectedTeam(e.target.innerText)
	}
		...
		<div className={style.App}>
			...
			<div className={style.Content}>
				<div className={style.Breadcrumb}>
					Teams >
					{loading ? <i className="fas fa-spinner fa-spin fa-2x" /> :
						<Dropdown
				...
				</div> : <TeamView team={selectedTeam} />}
			</div>
		</div>
	);
